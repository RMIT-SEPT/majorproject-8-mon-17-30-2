		...
		for (WorkerServiceBlueprint workerServiceId : workerIdList) {
			if (workerRepository.existsById(workerServiceId.getWorkerID())) {
				...
				for (Long serviceID : workerServiceId.getServiceID()) {
					...
					if (!((Collection<Service>) businessRepository.findById(businessId).get().getServices())
							.contains(tempService)) {
						throw new DataRetrievalFailureException(
								"Service ID " + serviceID + " is not in business ID " + businessId);
					}
				...
				throw new DataRetrievalFailureException(
						"Invalid worker ID " + workerServiceId.getWorkerID() + " not found");
			}
		}
